...
def showhighlightpath(graph,ax,pos,shortest_path):
    ...
    bg_image = plt.imread('./assets/bg.png')
...
def calculate_all_paths_to_polban(graph):
    ...
    for kosan in kosan_nodes:
        try:
            ...
            # Get price and gender attributes from graph node
            node_attrs = graph.nodes[kosan]
            ...
            # Get price and gender attributes even for unreachable nodes
            node_attrs = graph.nodes[kosan]
            price = node_attrs.get('price', 0)
            gender = node_attrs.get('gender', 'Unknown')
